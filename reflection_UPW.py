...
def decomposition(E_inc, cos_theta_inc, sin_theta_inc):
    ...
    E_inc_TE = yhat * E_inc_TE_amp
    E_inc_TM = (xhat * cos_theta_inc + zhat * sin_theta_inc) * E_inc_TM_amp

    return E_inc_TE, E_inc_TM, cos_beta, sin_beta



def reflected_field_TE(Gamma_r, cos_theta_inc, sin_theta_inc, E0 = E0, k1 = k_air):
    E_ref = lambda r : (yhat * Gamma_r * E0 
                        * np.exp(- complex(0,1) * k1 * (r[0] * sin_theta_inc - r[2] * cos_theta_inc)))
    return E_ref


def reflected_field_TM(Gamma_r, cos_theta_inc, sin_theta_inc, E0 = E0, k1 = k_air):
