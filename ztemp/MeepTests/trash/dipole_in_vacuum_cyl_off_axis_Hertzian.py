...
WAVELENGTH_UM = 1.0
PML_UM = 1.0
FARFIELD_RADIUS_UM = 1e3 * WAVELENGTH_UM
NUM_FARFIELD_PTS = 10
AZIMUTHAL_RAD = 0
POWER_DECAY_THRESHOLD = 1e-3
...
if __name__ == "__main__":
    ...
    parser.add_argument("dipole_pol", choices=["x", "y"], help="Polarization of the dipole: x (Er) or y (Ep)")
    parser.add_argument("dipole_pos_r", type=float, help="Radial position of the dipole (must be > 0)")
    args = parser.parse_args()

    if args.dipole_pos_r <= 0:
        raise ValueError("dipole_pos_r must be greater than zero")

    print(f"Running Hertzian dipole simulation at r = {args.dipole_pos_r}, pol = {args.dipole_pol}")

    e_total = np.zeros((NUM_FARFIELD_PTS, 3), dtype=np.complex128)
    h_total = np.zeros((NUM_FARFIELD_PTS, 3), dtype=np.complex128)
    flux_max = 0
    m = 0

    while True:
        e_plus, h_plus, e_minus, h_minus = dipole_in_vacuum(args.dipole_pol, args.dipole_pos_r, m)

        e_total += e_plus * cmath.exp(1j * m * AZIMUTHAL_RAD)
        h_total += h_plus * cmath.exp(1j * m * AZIMUTHAL_RAD)

        if m > 0:
            e_total += np.conj(e_minus) * cmath.exp(-1j * m * AZIMUTHAL_RAD)
            h_total += np.conj(h_minus) * cmath.exp(-1j * m * AZIMUTHAL_RAD)

        flux = flux_from_farfields(e_plus, h_plus)
        flux_max = max(flux_max, flux)
        power_decay = flux / flux_max
        print(f"[m = {m}] Flux = {flux:.3e}, Relative power = {power_decay:.3e}")

        if m > 0 and power_decay < POWER_DECAY_THRESHOLD:
            break
        m += 1

    radiation = radiation_pattern(e_total, h_total)
    plot_radiation_pattern(args.dipole_pol, radiation * FARFIELD_RADIUS_UM**2)

    if mp.am_master():
        np.savez("dipole_farfields_off_axis.npz",
                 ...
                 dipole_radiation_pattern=radiation,
                 e_field_total=e_total,
                 h_field_total=h_total,
                 m=m,
                 polar_rad=polar_rad)
