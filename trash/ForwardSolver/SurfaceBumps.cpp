...
std::unique_ptr<Surface> SurfaceBumps::mirrored(const Eigen::Vector3d& normal) const {
    auto mirroredSurface = std::make_unique<SurfaceBumps>(*this);
...
void SurfaceBumps::generateSurface() {
    // ------------- Generate grid -------------
    int N = resolution_;
