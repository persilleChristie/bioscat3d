...
UPWFieldCalculator::UPWFieldCalculator(const Eigen::Vector3d& waveVector,
...
void UPWFieldCalculator::computeFields(
    Eigen::MatrixXd& outE,
    Eigen::MatrixXd& outH,
    ...
    Eigen::Vector3d H0_real = (k_hat.cross(E0)) / (constants.mu0 * constants.c0);

    for (int i = 0; i < N; ++i) {
        const Eigen::Vector3d& x = evalPoints.row(i);
        double phase = k.dot(x);
        complex<double> phasor = exp(-j * phase);
