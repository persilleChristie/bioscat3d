...
pair<Vector3cd, Vector3cd> computeUPWField(const Vector3d& x, const Vector3d& k, const Vector3cd& E0) {
    complex<double> phase = exp(-constants.j * k.dot(x));
...
void FieldCalculatorUPW::computeFields(
    ...
    for (int i = 0; i < N; ++i) {
        ...
        auto [E, H] = computeUPWField(x, k, E0);
...
Vector3cd FieldCalculatorUPW::getEField(const Vector3d& x) const {
    auto [E, _] = computeUPWField(x, k, E0);
    return E;
}

Vector3cd FieldCalculatorUPW::getHField(const Vector3d& x) const {
    auto [_, H] = computeUPWField(x, k, E0);
    return H;
}
