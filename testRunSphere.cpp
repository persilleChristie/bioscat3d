...
// Assuming sphere() is already implemented and available
// void sphere(double radius, const Eigen::Vector3d& center, int num_points,
//             Eigen::MatrixXd& points, Eigen::MatrixXd& normals,
//             Eigen::MatrixXd& tau1, Eigen::MatrixXd& tau2);

// g++ -std=c++17 -O2 testRunSphere.cpp linearSystem.cpp spherePNT.cpp -o runSphere -I"D:/vcpkg/installed/x64-windows/include/eigen3" //
...
// =========================================
complex<double> E_r(const Eigen::Vector3d& x, double r, double Iel, double cos_theta, double eta0 = eta_air) {
    return (r == 0) ? 0.0 : eta0 * Iel * cos_theta / (2.0 * constants.pi * r * r) * (1.0 + 1.0 / (j * k0 * r)) * exp(-j * k0 * r);
}

complex<double> E_theta(const Eigen::Vector3d& x, double r, double Iel, double sin_theta, double eta0 = eta_air) {
    return (r == 0) ? 0.0 : (j * eta0 * Iel * sin_theta / (4.0 * constants.pi * r)) *
...
complex<double> H_phi(const Eigen::Vector3d& x, double r, double Iel, double sin_theta) {
    return (r == 0) ? 0.0 : j * k0 * Iel * sin_theta / (4.0 * constants.pi * r) * (1.0 + 1.0 / (j * k0 * r)) * exp(-j * k0 * r);
}
