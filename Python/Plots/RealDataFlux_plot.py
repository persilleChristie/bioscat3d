...
for filepath in files:
    ...
    # Updated regex pattern to extract all components including radius_ex
    match = re.search(
        r"surface(\d+)(variablePatchSize|constantPatchSize_\d+)_"
        r"(radius(?:Fixed_\d+|MC|Guard))_lambdamin_(\d+)_lambdamax_(\d+)_betanr_(\d+)",
        filename
    )
    ...
    surface_nr = match.group(1)
    patch_info = match.group(2)
    radius_info = match.group(3) 
    lambda_min = float(match.group(4)) * 10
    lambda_max = float(match.group(5)) * 10
    beta_nr = int(match.group(6))
    ...
    if patch_info == "variablePatchSize":
        ...
        match_patch = re.match(r"constantPatchSize_(\d+)", patch_info)
        patch_label = f"Constant Patch Size = {float(match_patch.group(1)) / 100}" if match_patch else patch_info

    # Format radius info
    # Format radius info
    if "Fixed_" in radius_info:
        radius_str = radius_info.split("_")[1]
        radius_val = int(radius_str) / 100  # Convert '025' -> 0.025
        radius_label = f"Radius Fixed = {radius_val:.3f}"
    elif "MC" in radius_info:
        radius_label = "Max Curvature Radius"
    else:
        radius_label = "Radius w/ Guard Rail"
    ...
    if beta_nr > 2:
        ...
        plt.xlabel("Polarization angle (rad)")
        plt.ylabel("Wavelength (nm)")
        plt.title(f"Surface {surface_nr}, {patch_label}, {radius_label}")

        save_path = f"RealData/flux_colormap_surface{surface_nr}_{patch_info}_{radius_info}.png"
        plt.tight_layout()
        plt.savefig(save_path, dpi=300)
        plt.close()
        print(f"Saved {save_path}")
        ...
        ax_flux.plot(lambdas, data[0], label=r'$\beta$ = 0.0')
        ax_flux.plot(lambdas, data[1], label=r'$\beta$ = $\pi$/2')
        ax_flux.set_title("Fluxes")
        ax_flux.set_xlabel("Wavelength λ (nm)")
        ax_flux.set_ylabel("Flux")
        ax_flux.grid(True)
        ax_flux.legend()
        fig.suptitle(f"Surface {surface_nr}, {patch_label}, {radius_label}")

        ax_ratio = axes[1]
        ratio = data[0] / data[1]
        ax_ratio.plot(lambdas, ratio, label='F1 / F2', color='darkblue')
        ax_ratio.axhline(1.0, ls='--', color='red', lw=1)
        ax_ratio.set_title("Flux Ratio")
        ax_ratio.set_xlabel("Wavelength λ (nm)")
        ax_ratio.set_ylabel("Ratio")
        ax_ratio.grid(True)

        plt.tight_layout()
        save_path = f"RealData/flux_surface{surface_nr}_{patch_info}_{radius_info}.png"
        plt.savefig(save_path, dpi=300)
        plt.close()
        print(f"Saved {save_path}")
